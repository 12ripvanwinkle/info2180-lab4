...
// Get the search query from the URL query parameter and sanitize it
$searchQuery = isset($_GET['query']) ? strtolower(trim($_GET['query'])) : '';

// Search for the superhero based on the query
$filteredSuperheroes = array_filter($superheroes, function($superhero) use ($searchQuery) {
    return strpos(strtolower($superhero['name']), $searchQuery) !== false || strpos(strtolower($superhero['alias']), $searchQuery) !== false;
});

// If a matching superhero is found, display the details
if (count($filteredSuperheroes) === 1) {
    $superhero = array_values($filteredSuperheroes)[0]; // Get the first match
